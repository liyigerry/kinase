...
class Protein(object):
    ...
    def get_atom_dict(self, removeHs=True, get_surf=False):
        ...
        atom_dict['is_backbone'] = np.array(atom_dict['is_backbone'], dtype=np.int64)
        #atom_dict['atom_name'] = atom_dict['atom_name']
        if get_surf:
            atom_dict['surface_mask'] = np.array([a.is_surf for a in self.get_heavy_atoms], dtype=np.int64)#self.get_surf_mask()
