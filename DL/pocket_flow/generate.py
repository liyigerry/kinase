...
from tools.sascorer import readFragmentScores, processMols, calculateScore
...
class Generate(object):
    ...
    def generate(self, data, num_gen=100, rec_name='recptor', with_print=True, root_path='gen_results'):
        ...
        sa_list = []
        valid_conuter = 0
        mol_dataFrame = pd.DataFrame(columns=['smiles', 'quality', 'QED', 'SA'])
        # 加载分数文件
        file_path = './tools/fpscores'
        sa_params = readFragmentScores(file_path)
        for i in tqdm(range(num_gen), desc=f'根据[{rec_name}]蛋白结构生成化合物中'):
            ...
            del data_clone
            if mol is not None:
                #print(len(mol.GetAtoms())) 分子
                mol.SetProp('_Name', 'No_%s-%s'%(valid_conuter, out_dir))
                smi = Chem.MolToSmiles(mol)
                # 计算分子质量
                # quality = MolWt(mol)
                quality_list.append(MolWt(mol))
                # 计算QED
                # qed = QED.qed(mol)
                ...
                Compute2DCoords(m)
                # sa = calculateScore(m)
                sa_list.append(calculateScore(m))
                if with_print:
                    print(smi)
                ...
                # with open(out_dir+'generated_score.csv','a') as smi_writer:
                #     smi_writer.write(
                #         smi+'\t'+
                #         str(quality)+'\t'+
                #         str(qed)+'\t'+
        ...
        mol_dataFrame.to_csv(out_dir+'metrics.csv')
